...
game.endTurn = function() {
  game.currentTurn += 1;
  io.of(game.id).emit('turn',game.currentTurn);
...
game.refreshStats = function() {
  ...
  io.of(game.id).emit('stats',stats);
};
...
game.console = function(type,msg) {
  io.of(game.id).emit('console',[type,msg]);
}

game.sockets = io.of(game.id).on('connection', function (socket) {
  if (game.players.length < game.maxPlayers) {
    ...
    if ((game.currentTurn == 0)&&(game.players.length == game.maxPlayers)) { game.endTurn(); } //start the game
    ...
    socket.on('end',function(data){
      socket.get('player',function(err,player){
        ...
        game.console('info',games[0].game.id);
      });
    });
...
var lobby = io.of('/0').on('connection',function(socket){  //initial connection from the client
  socket.emit('gameid','/'+games.length); //tell the client to join the last game
/*  if (games[games.length-1].game.players.length < games[games.length-1].game.maxPlayers) { //if the last game isn't full
    socket.emit('gameid',lastgame.id); //tell the client to join it
  } else {
    socket.emit('gameid',newGame()); //start a new game and redirect the client to it
  }*/
});
